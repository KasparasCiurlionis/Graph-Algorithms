...
void hierRecursive(struct node nodes[], int id, int *count);
...
int main()
{
    ...
    */
    ...
    for(int i=0;i<v2;i++) {
        ...
        for(int j=0;j<number;j++){
            fscanf(file, "%d", &temp);
            nodes2[i].branch=(bool *)malloc((v2) * sizeof(bool));
            for(int j=0;j<v2;j++){
    ...
    */
    ...
    printf("%d\n", v3);
    struct node nodes3[v3];

    for(int i=0;i<v3;i++){
        nodes3[i].next=(struct node **)malloc(v3 * sizeof(struct node *));
        nodes3[i].branch=(bool *)malloc((v3-1) * sizeof(bool));
        nodes3[i].id=i;
        nodes3[i].data=0;
        nodes3[i].size=0;
    }
    ...
    for(int i=0;i<v3;i++) {
        for(int j=0;j<v3;j++){
            nodes3[i].branch[j]=true;
            if(i!=j){
                nodes3[i].next[nodes3[i].size]=&nodes3[j];
                nodes3[i].size++;
            }
        }
    }
    ...
    int ran = rand() % (v3-1)+1;
    remove_nextarr_element(&nodes3[0], ran);
    remove_nextarr_element(&nodes3[ran], 0);

    /*
    for(int i=0;i<v3;i++){
        printf("[%d]",i);
        for(int j=0;j<nodes3[i].size;j++){
            printf("->(%d)",nodes3[i].next[j]->id);
        }
        printf("\n");
    }
    */
    ...
    t_end = gettime();
    printf("\n");
    printf("Fleury time: %.5f\n", t_end - t_start);


     for(int i=0;i<v3;i++) {
        for(int j=0;j<nodes3[i].size;j++){
            nodes3[i].branch[j]=false;
        }
    }

     t_start = gettime();
    hierholzer(nodes3, 0, v3);
    t_end = gettime();
    printf("\n");
    printf("Hierholzer time: %.5f\n", t_end - t_start);
...
for(int i=0;i<nodes[id].size;i++){
    if(nodes[id].branch[i]){
        ...
        //printf("id: %d, next: %d check: %d", id, nextId, checkBridge(nodes, id, nextId, v));
...
void hierholzer(struct node nodes[], int id, int v){
    ...
    Stack curr_path;
    initialize(&curr_path);
    Stack circuit;
    initialize(&circuit);

    pushStack(&curr_path, id);
    int curr_v = id;
    ...
    while(!isEmpty(&curr_path)){
        if (count[curr_v]>0)
        {
            ...
            for(int i=0;i<nodes[curr_v].size;i++){
                //printf("\nid:%d next:%d bool:%d\n", curr_v, nodes[curr_v].next[i]->id, nodes[curr_v].branch[i]);
                if(!nodes[curr_v].branch[i]){
                    next_v=nodes[curr_v].next[i]->id;
                    nodes[curr_v].branch[i]=true;
                    break;
                }
            }
            ...
            count[curr_v]--;
            count[next_v]--;
            for(int j=0;j<nodes[next_v].size;j++){
                if(nodes[next_v].next[j]->id==curr_v){
                    nodes[next_v].branch[j]=true;
                }
            }
